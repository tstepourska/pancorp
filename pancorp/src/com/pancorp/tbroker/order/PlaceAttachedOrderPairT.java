...
public class PlaceAttachedOrderPairT extends Thread 
{
	private static Logger lg = LogManager.getLogger(PlaceAttachedOrderPairT.class);
	...
	public PlaceAttachedOrderPairT(//int orderId, 
			Contract c, 
			double limPr, 
			int q, 
			String a, 
	...
	public void invoke(){
		try {
			...
			lg.info("action: " + action + ", limit: " + adjustedLimPrice+", stopLoss: " + stopLoss);
			
				List<Order> orderPair = createAttachedOrderPair( 
						action, quantity, adjustedLimPrice, stopLoss);
				lg.info("attached order pair created, submitting orders");
			
				orderPairSubmit(wrapper.getClient(), orderPair, m_contract);	
	 ...
	 */
	private void orderPairSubmit(EClientSocket client, List<Order> orderPair, Contract contract ) throws InterruptedException,Exception {
		String fp = "orderPairSubmit: ";
		int parentOrderId = 0;
		int cnt = 0;
		...
		this.manager.setOpenOrderList(orderPair);
	}
	 ...
	 * @return
	 */
	private List<Order> createAttachedOrderPair(//int parentOrderId, 
			String action, double quantity, double limit, double stopLossPrice){
