...
{
	private Logger lg = LogManager.getLogger(ForexBroker.class); 
	...
	public ForexBroker(Contract c, int rid, DataFactory df, String tfu, BrokerManagerEWrapperImpl wr, BrokerManager mgr) throws Exception {
		...
		this.strategy = new StrategyFX1(calc);
		//subscribe to data
...
/*	private Candle initTick(Bar t){
	...
	/*	try{
		...
		try {
			switch(mode){
			case Constants.MODE_OPENING:
				strategy.evaluateEntry(candles);	
				break;
			case Constants.MODE_CLOSING:
				strategy.evaluateExit(candles);
				break;
				default:
			}
		}
			...
			lg.trace("Caught OpenPositionEvent");
				...
				if(manager.setOrderPlaced(true)){	
					...
					PlaceOrderBracketT placeOrderThread = new PlaceOrderBracketT(//wrapper.getCurrentOrderId(), 
							...
							Constants.DEFAULT_FOREX_QUANTITY, 	// position size (quantity currency units), default 20000
